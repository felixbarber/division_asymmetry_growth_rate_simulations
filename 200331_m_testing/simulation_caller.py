...
temp_base_path = '/n/holyscratch01/murray_lab/fbarber/20_growth_rate_project/200331_m_testing/outputs/'
...
observables = ['MD corr', 'MD corr p val', 'GR num', 'GR vol', 'td_std', 't_av', 'vb_std', 'v_av', 'linear slope', 'm factor']
...
prog_path = '/n/holyscratch01/murray_lab/fbarber/20_growth_rate_project/200331_m_testing/outputs/progress_tracker_{0}.npy'.format(temp_ind)
if os.path.exists(prog_path):
    ...
    # left off
out_path = '/n/holyscratch01/murray_lab/fbarber/20_growth_rate_project/200331_m_testing/outputs/output_vals_{0}.npy'.format(temp_ind)
...
for ind in temp3:
    ...
    for i6 in range(2):
        ...
        xv = len([obj for obj in c if obj.celltype == i6 and obj.exists])*1.0
        output_val[ind1[0], ind1[1], ind1[2], ind1[3], ind1[4], i6, 9] = xv  # m factor
    i6 = 2
    ...
    temp = scipy.stats.linregress(xv, yv)
    output_val[ind1[0], ind1[1], ind1[2], ind1[3], ind1[4], i6, 8] = temp[0]  # vb vd linear slope
    xv = len([obj for obj in c if obj.celltype == 1 and obj.exists])*1.0
    yv = len([obj for obj in c if obj.celltype == 0 and obj.exists])*1.0
    output_val[ind1[0], ind1[1], ind1[2], ind1[3], ind1[4], i6, 9] = (xv-yv)/(xv+yv)  # m factor
