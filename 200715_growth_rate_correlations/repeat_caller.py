...
job_number=65980648
path1='/n/holyscratch01/murray_lab/fbarber/20_growth_rate_project/200715_growth_rate_correlations/failed_{0}.txt'.format(job_number)
...
temp_ind1 = 1500  # changing the upper index to make it match what was previously used

# from now on everything is the same as simulation_caller.py
# ran with job number 66080887
...
# ran twice. Second time job ID was 65980648
...
temp_base_path = '/n/holyscratch01/murray_lab/fbarber/20_growth_rate_project/200715_growth_rate_correlations/outputs/'
...
prog_path = '/n/holyscratch01/murray_lab/fbarber/20_growth_rate_project/200715_growth_rate_correlations/outputs/progress_tracker_{0}.npy'.format(temp_ind)
if os.path.exists(prog_path):
    ...
    # left off
out_path = '/n/holyscratch01/murray_lab/fbarber/20_growth_rate_project/200715_growth_rate_correlations/outputs/output_vals_{0}.npy'.format(temp_ind)
...
for ind in temp3:
    ...
    par_vals['a']=gr_corr_a[ind1[5]]
    par_vals['b']=1.0-par_vals['a']  # ensuring that the average growth rate is consistently set to 1.
    ...
    for i6 in range(2):
        ...
        # print temp
        output_val[ind1[0], ind1[1], ind1[2], ind1[3], ind1[4], ind1[5], i6, 0] = temp[0]  # PCC
        output_val[ind1[0], ind1[1], ind1[2], ind1[3], ind1[4], ind1[5], i6, 1] = temp[1]  # PCC pval
        if i6 == 0:
            temp = scipy.stats.linregress(obs[1][400:], np.log(obs[10][400:]))
            output_val[ind1[0], ind1[1], ind1[2], ind1[3], ind1[4], ind1[5], i6, 2] = temp[0]  # GR num
            temp = scipy.stats.linregress(obs[1][400:], np.log(obs[12][400:]))
            output_val[ind1[0], ind1[1], ind1[2], ind1[3], ind1[4], ind1[5], i6, 3] = temp[0]  # GR vol
        if i6 == 1:
        ...
        output_val[ind1[0], ind1[1], ind1[2], ind1[3], ind1[4], ind1[5], i6, 4] = np.std(xv)  # td_std
        output_val[ind1[0], ind1[1], ind1[2], ind1[3], ind1[4], ind1[5], i6, 5] = np.mean(xv)  # td_av
        xv = [obj.vb for obj in c if obj.celltype == i6 and obj.tb>400* par_vals['dt'] * np.log(2)/par_vals['lambda']]
        output_val[ind1[0], ind1[1], ind1[2], ind1[3], ind1[4], ind1[5], i6, 6] = np.std(xv)  # vb_std
        output_val[ind1[0], ind1[1], ind1[2], ind1[3], ind1[4], ind1[5], i6, 7] = np.mean(xv)  # vb_av
        ...
        output_val[ind1[0], ind1[1], ind1[2], ind1[3], ind1[4], ind1[5], i6, 8] = temp[0]  # vb vd linear slope
    i6 = 2
    ...
    output_val[ind1[0], ind1[1], ind1[2], ind1[3], ind1[4], ind1[5], i6, 3] = temp[0]  # GR num
