...
list2 = [4, 16]  # budding growth morphology
...
# modeltype 16 is the same as modeltype 7, except it describes a budding growth morphology. We still have a linear
# growth policy model f(vb) =2aD+2*(1-a)*vb where a=0 gives timer, a=0.5 gives adder and a=1 gives
# sizer.
def fn(L, tm, td):
...
class Cell(object):
    ...
    def grow_g1(self, par1):  # par1 contains all details for the specified cell
        ...
        if par1['modeltype'] == 0:
            ...
            self.t_div = self.tb+self.t_grow
        elif par1['modeltype'] in [7,16]:  # linear growth policy tuning between timer adder and sizer
            temp = np.random.normal(0.0, 1.0, size=2)
            self.gr = par1['lambda'] * (1 + temp[0] * par1['lambda_std'])
            if self.gr<0:
    ...
    def grow(self, par1):  # par1 contains all details for the specified cell
        ...
        if par1['modeltype'] == 0:
            ...
            self.wd = self.wb+par1['w_synth'][0]*self.t_g2+par1['w_synth'][1]*(self.vd-self.vs)/self.gr
            self.t_div = self.tb+self.t_grow
        elif par1['modeltype']  in [7,16]:  # inhibitor dilution with synth rate = a +bv
            temp = np.random.normal(0.0, 1.0, size=2)
            self.gr = par1['lambda'] * (1 + temp[0] * par1['lambda_std'])
            if self.gr<0:
                # if cells would have a negative
