...
# to select the ones that failed for an unknown reason:
# sacct -j {jobid#} -o "JobID%16,Partition,State" | grep serial_re | grep -e 'OUT_OF_ME' -e FAILED -e TIMEOUT | awk '{print $1}' | awk -F _ '{print $2}' >> failed_unknown_reason_{jobid#}.txt
...
job_number=4267922
path1='/n/holyscratch01/murray_lab/fbarber/20_growth_rate_project/201007_correlations_validation_theory_model_budding/failed_{0}.txt'.format(job_number)
...
# # should take around 7 hours.49848679.50046975.4309731

par_vals = {'nstep': 900, 'dt': 0.01, 'v_init': 1.0, 'modeltype': 17, 'delta': delta[0], 'lambda': l[0]}
...
temp_base_path = '/n/holyscratch01/murray_lab/fbarber/20_growth_rate_project/201007_correlations_validation_theory_model_budding/outputs/'
...
prog_path = '/n/holyscratch01/murray_lab/fbarber/20_growth_rate_project/201007_correlations_validation_theory_model_budding/outputs/progress_tracker_{0}.npy'.format(temp_ind)
if os.path.exists(prog_path):
    ...
    # left off
out_path = '/n/holyscratch01/murray_lab/fbarber/20_growth_rate_project/201007_correlations_validation_theory_model_budding/outputs/output_vals_{0}.npy'.format(temp_ind)
