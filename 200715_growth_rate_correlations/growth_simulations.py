...
# modeltype 7 is a linear growth policy model f(vb) =2aD+2*(1-a)*vb where a=0 gives timer, a=0.5 gives adder and a=1 gives
# sizer.
...
# modeltype 15 is a tunable asymmetrically dividing cell type with the growth policy t=(ln(2)+alpha*ln(delta/vb)/lambda+eta
# so that alpha=0 is a timer, alpha=0.5 is an adder, and alpha=1.0 is a sizer. This model agrees to first order with modeltype 7

# modeltype 16 is the same as modeltype 7 except that we allow for a growth rate correlation between closely related
# cells of the form lambda_d=a*lambda_m+b+epsilon where epsilon is a random variable (see lin and amir, 2017 for details).
# For convenience we will typically set b=1,
def fn(L, tm, td):
    return np.exp(-L * td) + np.exp(-L * tm) - 1.0

# models in which I track Whi5
global list_whi5
list_whi5 = [5, 8, 9, 10, 11, 12, 14]
...
class Cell(object):
    ...
    def __init__(self, birth_parameters, temp_params):  # birth parameters = [tb, celltype, parent, vb, parent_current]
        ...
        self.nextgen=None  # placeholder for now
        self.daughter=None  # placeholder for now
        # small in model 13
    ...
    def grow_g1(self, par1):  # par1 contains all details for the specified cell
        ...
        if par1['modeltype'] == 0:
            ...
            self.gr = par1['lambda'] * (1 + temp[0] * par1['lambda_std'])
            if self.gr<0:
                # if cells would have a negative
                # growth rate, we effectively make their growth rate so low as to be irrelevant within the length of
                # these simulations
                    # temp[0]=np.random.normal(0.0, 1.0, size=1)[0]
                    self.gr=0.001*np.log(2)/(par1['dt']*par1['nstep'])
            self.t_grow = max(np.log((2*par1['alpha']*par1['delta']+2*(1-par1['alpha'])*self.vb)/self.vb) / self.gr +
                              par1['td_std']*temp[1]/par1['lambda'], par1['dt'])
            self.t_div = self.tb + self.t_grow
            self.vd = self.vb * np.exp(self.gr * self.t_grow)
        ...
        # self.t_div = self.tb + np.amax([self.t_grow, 0.0])
            ...
            self.wd = par1['f']*self.wb + par1['delta']  # no noise in inhibitor production, and only a fraction f
            # of inhibitor at birth is kept
        elif par1['modeltype'] == 15:  # nonlinear growth policy tuning between timer adder and sizer
            temp = np.random.normal(0.0, 1.0, size=2)
            self.gr = par1['lambda'] * (1 + temp[0] * par1['lambda_std'])
            if self.gr < 0:
                # if cells would have a negative
                ...
                # temp[0]=np.random.normal(0.0, 1.0, size=1)[0]
                self.gr = 0.001 * np.log(2) / (par1['dt'] * par1['nstep'])
            self.t_grow = max((np.log(2)+par1['alpha']*np.log(par1['delta']/self.vb)) / self.gr +
                              par1['td_std'] * temp[1] / par1['lambda'], par1['dt'])
            self.t_div = self.tb + self.t_grow
            self.vd = self.vb * np.exp(self.gr * self.t_grow)
        elif par1['modeltype']==16:
            temp=np.random.normal(0.0,1.0,size=2)

            # note since the grow_g1 function doesn't assume the existence of a parent cell, we define the growth rate
            # here for these seeded cells independently of the parents. lambda_std is the desired CV in growth rate.
            self.gr=(par1['b']/(1-par1['a']))*(1.0+par1['lambda_std']*temp[0])
            if self.gr<0:
                # if cells would have a negative
                # growth rate, we effectively make their growth rate so low as to be irrelevant within the length of
                # these simulations
                    # temp[0]=np.random.normal(0.0, 1.0, size=1)[0]
                    self.gr=0.001*np.log(2)/(par1['dt']*par1['nstep'])
            # note that the normalization of td_std is done here relative to the average growth rate b/(1-a), rather
            # than the
            self.t_grow = max(np.log((2*par1['alpha']*par1['delta']+2*(1-par1['alpha'])*self.vb)/self.vb) / self.gr +
                              par1['td_std']*temp[1]/(par1['b']/(1-par1['a'])), par1['dt'])
    ...
    def grow(self, par1):  # par1 contains all details for the specified cell
        ...
        if par1['modeltype'] == 0:
            ...
            temp = np.random.normal(0.0, 1.0, size=2)
            # lambda_std is the desired CV in growth rate, so that the variance in noise added in each cell cycle
            # is normalized to give the desired CV in growth rate.
            self.gr = par1['a'] * self.parent.gr + par1['b'] + par1['lambda_std'] *np.sqrt(1-par1['a']**2)*(par1['b']/(1-par1['a']))* temp[0]
            if self.gr < 0:
                # if cells would have a negative
                # growth rate, we effectively make their growth rate so low as to be irrelevant within the length of
                # these simulations
            ...
            # than the
            self.t_grow = max(
                np.log((2 * par1['alpha'] * par1['delta'] + 2 * (1 - par1['alpha']) * self.vb) / self.vb) / self.gr +
                par1['td_std'] * temp[1] / (par1['b'] / (1 - par1['a'])), par1['dt'])
...
def starting_popn(par1):
    ...
    elif par1['modeltype'] == 7 or par1['modeltype']==13 or par1['modeltype'] == 15 or par1['modeltype']==16:
        vm = par1['delta'] / (1 + par1['r'])
        vd = par1['r'] * par1['delta'] / (1 + par1['r'])
...
def next_gen(index, f, t, par1,temp_ind=-1):
    ...
    if par1['modeltype'] in list2:
        ...
        frac1 = par1['r']/(1+par1['r'])  # not a budding growth morphology, simply asymmetric division.

    # add new cell for new cycle of mother cell.
    temp = [t, 0, weakref.proxy(f[index]), (1-frac1) * f[index].vd, None]
    if par1['modeltype'] == 5 or par1['modeltype'] == 6 or par1['modeltype'] == 8 or par1['modeltype']==14:
...
def discr_time_1(par1, starting_pop):
    ...
    nstep = par1['nstep']
    if par1['modeltype']==16: # in this case the average growth rate is not lambda, but b/(1-a)
        tvec = np.linspace(0.0, nstep * par1['dt'] * np.log(2) / (par1['b']/(1-par1['a'])), nstep + 1)
    else:
...
def starting_popn_seeded(temp_cells, temp_params, discr_time=True):
    ...
    indices = np.random.randint(low=0, high=len(temp_cells), size=2*par['num_s'])
    ...
    if discr_time:
        ...
        if temp_params['modeltype']==16:
            t_max = nstep * temp_params['dt'] * np.log(2) / (temp_params['b']/(1-temp_params['a']))
        else:
